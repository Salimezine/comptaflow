...
for fname in sorted(os.listdir(uploads_dir)):
    ...
    try:
        ...
        # Work per-line to avoid cross-line matching
        lines = text.split('\n')
        ...
        for line in lines:
            # HT 0%: "3 127,906 0%"
            if ht0 == 0:
                m = re.search(r'^([\d][\d ,.]+?)\s+0%\s', line)
                if m:
                    val = m.group(1).strip().replace(' ', '').replace(',', '.')
                    try: ht0 = float(val)
                    except: pass

            # HT 19% + TVA: "659,924 19% 125,386"
            if ht19 == 0:
                m = re.search(r'^([\d][\d ,.]+?)\s+19%\s+([\d ,.]+)', line)
                if m:
            ...
            # Timbre
            if timbre == 1.0:
                m = re.search(r'TIMBRE\s+FIS\.\s*:\s*([\d ,.]+)', line)
                if m:
                    val = m.group(1).strip().replace(' ', '').replace(',', '.')
            ...
            # Net TTC
            if ttc == 0:
                m = re.search(r'NET\s+T\.T\.C\.\s*([\d ,.]+)', line)
                if m:
                    val = m.group(1).strip().replace(' ', '').replace(',', '.')
                    try: ttc = float(val)
                    except: pass
